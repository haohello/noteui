...
import objectKeyToValues from '../utils/objectKeysToValues'
...
const mergeThemes = (...themes: ThemeInput[]): ThemePrepared => {
  return themes.reduce<ThemePrepared>(
    (acc: ThemePrepared, next: ThemeInput) => {
      ...
      /**
       * breakpoints cannot be merged, so we will use the outer-most theme's breakpoints
       */
      if (!acc.breakpoints) {
        acc.breakpoints = next.breakpoints
      }
